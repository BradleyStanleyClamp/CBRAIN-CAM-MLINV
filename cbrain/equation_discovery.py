...
def subsampler(ind,x,y,xRH,xB,xLHFns,hyam,hybm,
    ...
    xt, yt, xRHt, xBt, xLHFnst = (Optional) Same but for the test set
    ...
    if ind is not None:
        ...
        # Do not sub-sample the training nor the test set
        x_sub = x; y_sub = y; xRH_sub = xRH; xB_sub = xB; xLHFns_sub = xLHFns;
        if xt is not None:
            for iclim,clim in enumerate(climates):
                xt_sub[clim] = xt[clim]; yt_sub[clim] = yt[clim];
                xRHt_sub[clim] = xRHt[clim]; xBt_sub[clim] = xBt[clim];
                xLHFnst_sub[clim] = xLHFnst[clim]
    # Pressure
...
def range_normalizer(x_train,scalar_keys,vector_keys,x_test=None,Norm=None):
    """
...
def dic_to_array(key_array,x_train_range,y_train,scale_dict,x_test_range=None,y_test=None):
    """
...
def SFS_poly(features,X_train,Y_train,min_features,max_features,cv,X_test=None,Y_test=None):
    """
    ...
    for no_features in np.arange(min_features, max_features):   
        ...
        if X_test is not None: dict_exp['mse_test'] = mse_test
        print(dict_exp,'\n')
